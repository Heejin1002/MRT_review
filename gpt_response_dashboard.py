...
- 리뷰 내용을 정확히 이해한 뒤, 위 [리뷰 인용 – 최소화]와 [자연스러운 한국어 문체]를 지켜 답변할 것
- 리뷰 요지를 반영하되 우리 말로 짧게 풀어서 문맥에 맞는 답변만 작성 (굳이 리뷰 문장을 많이 반복하지 말 것)
    ...
    def generate_response(self, product_title, review_content):
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4o-mini", 
                messages=[
                    {"role": "system", "content": "당신은 여행사 몽키트래블의 고객 서비스 담당자입니다. 모든 답변은 구어체에 가깝고 자연스러운 한국어로 작성하며, '-했답니다' 같은 어색한 표현은 사용하지 않습니다. 고객 리뷰 문장을 그대로 인용하지 말고, 내용을 이해한 뒤 우리 말로 짧게 풀어 문맥에 맞게 답하세요."},
    ...
    if not filtered_df.empty:
        for idx, row in filtered_df.iterrows():
            with st.container():
                ...
                # GPT 답변 줄바꿈: 화면 표시용으로 \n → <br>
                gpt_answer_display = (row.get('GPT답변', 'N/A') or 'N/A').replace('\n', '<br>')
                ...
                <div class="review-card" style="border-left: 5px solid {partner_color}; background-color: {partner_bg_color};">
                    ...
                    <div class="gpt-response" style="white-space: pre-wrap;">
