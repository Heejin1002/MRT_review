...
- 고객이 한 말을 그대로 인용할 때만 따옴표 사용. 나머지는 인용하지 말고 자연스럽게 풀어서 쓸 것.
...
[이모티콘 예시 1] 가족·아이·만족 후기
→ 안녕하세요, 몽키트래블입니다 :) 한국인 사장님과 직원분들의 친절한 설명 덕분에 아이들과 즐거운 시간 보내셨다니 정말 다행이었네요 👨‍👩‍👧‍👦😊 시설과 운영도 만족스러우셨고 주변에까지 소개해주셨다니 더욱 감사한 마음입니다 ✨ 소중한 후기 감사합니다 😊
[이모티콘 예시 2] 일정 피로 + 일부 불편 공감 후기
→ 안녕하세요, 몽키트래블입니다 :) 이른 일정으로 피곤하셨을 텐데도 색다른 투어로 느껴지셨다니 다행이에요 😊 수상시장 배 이용 중 불편함을 느끼신 점은 공감되며, 더 나은 안내가 될 수 있도록 참고하겠습니다 🙏 소중한 후기 감사합니다 😊
...
- 리뷰 내용을 정확히 이해하고 위 [자연스러운 한국어 문체]를 지켜 답변할 것
    ...
    def generate_response(self, product_title, review_content):
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4o-mini", 
                messages=[
                    {"role": "system", "content": "당신은 여행사 몽키트래블의 고객 서비스 담당자입니다. 모든 답변은 구어체에 가깝고 자연스러운 한국어로 작성하며, '-했답니다' 같은 어색한 표현은 사용하지 않습니다."},
